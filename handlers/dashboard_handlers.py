...
def _format_dashboard_message(data: dict) -> str:
    ...
    # Отказы судей после одобрения
    refusals = data.get('refusals', {})
    message += f"🚫 <b>ОТКАЗЫ ПОСЛЕ ОДОБРЕНИЯ:</b>\n"
    message += f"   • Отказов за сезон: {refusals.get('season_refusals', 0)}\n"
    message += f"   • Доля от всех одобрений: {refusals.get('season_refusal_pct', 0)}%\n"
...
def _format_detailed_stats_message(data: dict) -> str:
    ...
    message += f"🚫 <b>ОТКАЗЫ ПОСЛЕ ОДОБРЕНИЯ (СЕЗОН):</b>\n"
    message += f"   • Отказов: {refusals.get('season_refusals', 0)}\n"
    message += f"   • Доля от всех одобрений: {refusals.get('season_refusal_pct', 0)}%\n"
    message += f"   • Ответственность судей: {refusals.get('responsibility_label', '—')} ({refusals.get('responsibility_score', 0)} / 100)\n"
    if monthly_refusals:
        message += "   • По месяцам:\n"
        for month_item in monthly_refusals[:6]:
            message += (
                ...
                f"{month_item.get('refusals', 0)} отказов\n"
            )
    message += "\n"
