...
class DashboardService:
    ...
    def _get_current_season_end(self) -> date:
        ...
        _, end = get_season_date_range(get_current_season_key())
        return end
    ...
    async def _get_refusals_stats(self) -> Dict[str, Any]:
        """Статистика по отказам судей после одобрения заявки."""
        try:
            ...
            season_refusals = (
                self.session.query(func.count(RegistrationCancellation.cancellation_id))
                .filter(approved_filter)
                .scalar()
                or 0
            )
            season_approved_assignments = count_season_approved_assignments(
                self.session, season_start, season_end, season_filter
            )
            season_refusal_pct = (
            ...
            responsibility_label = get_responsibility_label(season_refusal_pct)
            ...
            ).filter(approved_filter).group_by('month_key').all()
            ...
            return {
                'season_refusals': 0,
                'season_approved_assignments': 0,
                'season_refusal_pct': 0.0,
